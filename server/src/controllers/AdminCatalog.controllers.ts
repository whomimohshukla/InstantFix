...
export const adminAddServiceMediaCtrl = async (req: Request, res: Response) => {
	...
	const result = await adminCatalogService.addServiceMedia(
		serviceId,
		req.body
	);
	if (result.ok) {
...
export const adminDeleteServiceMediaCtrl = async (
	req: Request,
	res: Response
) => {
	const serviceId = req.params.id;
	const mediaId = req.params.mediaId;
	const result = await adminCatalogService.deleteServiceMedia(
		serviceId,
		mediaId
	);
	if (result.ok) {
...
export const adminUpdateCategoryCtrl = async (req: Request, res: Response) => {
	const result = await adminCatalogService.updateCategory(
		req.params.id,
		req.body
	);
	if (result.ok) {
...
export const adminUpdateServiceCtrl = async (req: Request, res: Response) => {
	const result = await adminCatalogService.updateService(
		req.params.id,
		req.body
	);
	if (result.ok) {
